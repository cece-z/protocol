...
export class SamplerOperations {
    ...
    public mint(takerToken: string, amount: BigNumber, isDeposit: boolean): BatchedOperation<void> {
        return {
            encodeCall: () => this._samplerContract.mint(takerToken, amount, isDeposit).getABIEncodedTransactionData(),
            handleCallResults: (_callResults: string) => {
                return;
            },
            handleRevert: (data: string) => {
                throw new Error(`mint for ${takerToken} reverted: ${data}`);
            },
        };
    }
    ...
    public getUniswapV2SellQuotes(
        ...
        return new SamplerContractOperation({
            ...
            contract: this._samplerContract,
            function: this._samplerContract.sampleSwapsFromUniswapV2,
            ...
            callback: (callResults: string, fillData: UniswapV2FillData): BigNumber[] => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData<[BigNumber[], BigNumber[]]>(
                    'sampleSwapsFromUniswapV2',
                    callResults,
                );
                console.log(
                    source,
                    tokenAddressPath,
                    gasUsed.map(g => g.toString(10)),
                );
                return samples;
            },
        });
